...
class SettingRepository extends RepositoriesAbstract implements SettingInterface
{
     ...
     * {@inheritDoc}
     */
    public function getWhereRow($array=[])
    {
        $data = $this->model
            ->where($array)
            ->orderBy('created_at', 'desc');
     ...
     * {@inheritDoc}
     */
    public function Update($whereArray=[],$recordArray=[])
    {
        $data = $this->model
            ->where($whereArray);
        return $data->update($recordArray);
    }
    ...
    {
        $data = $this->model->select('referral_commission.*')
            ->with('slugable')
            ->orderBy('referral_commission.created_at', 'desc');

        if ($active) {
            $data = $data->where('referral_commission.status', BaseStatusEnum::PUBLISHED);
        }
